...
in_x_pos = round(2850/scale)
in_y_pos =  mars_map.shape[0] - round(6400/scale)

fin_x_pos = round(3150/scale)
fin_y_pos = mars_map.shape[0] - round(6800/scale)

in_pos = tuple(np.array([in_y_pos, in_x_pos]))
fi_pos = tuple(np.array([fin_y_pos, fin_x_pos]))    
height = .25

rover = MarsRover(in_pos, fi_pos, mars_map, height)

path_x = (np.array(rover.get_x()) * scale) 
path_y = (mars_map.shape[0] - np.array(rover.get_y())) * scale
path_z = np.array(rover.get_z())

x = new_scale*np.arange(image_data.shape[1])
y = new_scale*np.arange(image_data.shape[0])
X, Y = np.meshgrid(x, y)

print(rover.distance*scale)
